...
Правильная логика (по user'у):
  1. OB 1h сформирован — режим слежения включается.
  2. Ждём retest OB (цена wick'ом коснулась body).
  3. С момента retest — гонка между:
     - VC-элемент образуется на 15m (X-1 for 1h POI) (правильное направление, zone overlaps body OB)
...
TF_MS_1H = 3600 * 1000
TF_MS_RETEST = 15 * 60 * 1000   # retest/break TF = 15m (VC tf = X-1 for 1h POI)
...
def find_retest_ts_batch(ob_df: pd.DataFrame, bars: pd.DataFrame, cap_ms: int) -> np.ndarray:
    """Первое время (wick на TF retest) когда цена вошла в OB zone.
    Bars должны быть агрегированы на нужный TF (15m для 1h POI)."""
    ...
    for i, r in enumerate(ob_df.itertuples()):
        cur_close = int(r.cur_open) + TF_MS_1H
...
def find_invalidation_ts_batch(ob_df: pd.DataFrame, bars: pd.DataFrame, cap_ms: int) -> np.ndarray:
    """Invalidation = первый wick на TF retest прошёл за OB zone (реальное пробитие POI).
       LONG:  bar.low < zone_lo
       SHORT: bar.high > zone_hi
       Bars агрегированы на нужный TF (15m для 1h POI).
    """
    result = np.full(len(ob_df), 2**62, dtype=np.int64)
    m1_ts = bars["ts_ms"].values.astype(np.int64)
    m1_low = bars["low"].values.astype(np.float64)
...
def check_valid_vc(ob_df: pd.DataFrame, retest_ts: np.ndarray, brk_ts: np.ndarray,
    ...
    """VC born_ts:
      - в окне [retest_ts, window_end) где window_end = min(body_break_ts, cap_end)
        cap_end = cur_open + TF_MS_1H + cap_ms (safety cap если тела не пробило)
    ...
    for i, r in enumerate(ob_df.itertuples()):
        ...
        cap_end = cur_open + TF_MS_1H + cap_ms
...
def main() -> None:
    ...
    # VC pools: только 15m (X-1 по канону для 1h POI)
    print("Building VC pools (15m only) ...")
    pools = {}
    pools["rb_15m"] = build_element_born(events, "rb", "15m")
    pools["fvg_15m"] = build_element_born(events, "fvg", "15m")
    pools["snr_15m"] = build_element_born(events, "breaker_block", "15m")
    print(f"  1h:  RB={len(pools['rb_15m']):,}  FVG={len(pools['fvg_15m']):,}  SNR={len(pools['snr_15m']):,}")

    cap_ms = WATCH_CAP_H * 3600 * 1000

    # Aggregate 1m → TF_MS_RETEST (15m) для retest/invalidation
    ...
    for name, cohort in [("sweep-only", sweep_only), ("canonical", canonical)]:
        print(f"\n[{name}] compute retest_ts + invalidation (wick за OB zone на {TF_MS_RETEST//60000}m) ...")
        ...
        cohort["vc_rb"]  = check_valid_vc(cohort, rt, bt, pools["rb_15m"],  cap_ms)
        cohort["vc_fvg"] = check_valid_vc(cohort, rt, bt, pools["fvg_15m"], cap_ms)
        cohort["vc_snr"] = check_valid_vc(cohort, rt, bt, pools["snr_15m"], cap_ms)
        ...
        cohort["vc_rb_ts"]  = get_vc_born_ts(cohort, rt, bt, pools["rb_15m"],  cap_ms)
        cohort["vc_fvg_ts"] = get_vc_born_ts(cohort, rt, bt, pools["fvg_15m"], cap_ms)
        cohort["vc_snr_ts"] = get_vc_born_ts(cohort, rt, bt, pools["snr_15m"], cap_ms)

    print()
    print("═" * 105)
    print(f"Этап 4 — RACE (retest → VC/break) на 15m. VC zone overlaps body OB.")
    print("═" * 105)
    ...
    print()
    row("valid RB 15m",   sweep_only["vc_rb"],   canonical["vc_rb"])
    row("valid FVG 15m",  sweep_only["vc_fvg"],  canonical["vc_fvg"])
    row("valid SNR 15m",  sweep_only["vc_snr"],  canonical["vc_snr"])
    print()
