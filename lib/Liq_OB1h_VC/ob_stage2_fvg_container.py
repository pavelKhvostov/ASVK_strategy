...
HTF: 4h/6h/12h/1D/2D/3D/1W. Target TF: 1h (1h/2h — не HTF).
...
FILTER 1 — IDM (first-touch wickfill свежей HTF FVG):
  ...
  cur bar — самый первый 1h бар тронувший FVG (first_wick_touch_ts == cur_open).
...
BTC 8.5 лет: (Liq_OB1h_VC — цифры зависят от прогона).
"""
...
STAGE_DIR = DATA_DIR / "liq_ob1h_vc"

TFS = ["4h", "6h", "12h", "1d", "2d", "3d", "1w"]  # HTF only для 1h POI (всё выше 2h)
ATR_MULTIPLIER = 0.5
ATR_PERIOD = 14
TF_MS_1H = 3600_000
...
def find_latest_stage1(symbol: str) -> Path:
    files = sorted(STAGE_DIR.glob(f"ob_canonical_1h_v1_{symbol}_*.parquet"),
                   key=lambda p: p.stat().st_mtime)
    files = [p for p in files if "_config" not in p.stem]
    if not files:
        raise FileNotFoundError(f"No ob_canonical_1h_v1_{symbol}_*.parquet found in {STAGE_DIR}")
    return files[-1]


def compute_first_wick_touch(fvgs: pd.DataFrame, bars_1h: pd.DataFrame) -> np.ndarray:
    """Для каждой FVG вернуть ts_ms первого 1h бара range которого пересёкся с FVG после born.
    Если ни один не тронул — 2**62."""
    if len(fvgs) == 0:
        return np.array([], dtype=np.int64)
    bar_ts = bars_1h["ts_ms"].values.astype(np.int64)
    bar_l = bars_1h["low"].values.astype(np.float64)
    bar_h = bars_1h["high"].values.astype(np.float64)
...
def compute_active_bounds_at(zone_lo, zone_hi, direction, born_ts, target_ts,
    ...
    """Compute active portion of FVG at target_ts.
    SHORT FVG: consumed from below. mit_level = max wick_high внутри зоны с born.
               active = [mit_level, zone_hi]. Полное mit если mit_level >= zone_hi.
    LONG FVG: consumed from above. mit_level = min wick_low внутри зоны.
              active = [zone_lo, mit_level]. Полное mit если mit_level <= zone_lo.
    Returns (active_lo, active_hi, fully_mitigated).
    """
...
def aggregate_1h_bars(csv_path: Path) -> pd.DataFrame:
    ...
    df["bucket"] = (df["ts_ms"] // TF_MS_1H) * TF_MS_1H
...
def load_atr_1h(csv_path: Path) -> pd.DataFrame:
    """Compute ATR(14) on 1h aggregated from 1m CSV. Return DataFrame with ts_ms, atr14."""
    df = pd.read_csv(csv_path, usecols=["open_time","high","low","close"])
    df["ts_ms"] = pd.to_datetime(df["open_time"], utc=True, format="ISO8601").astype("datetime64[ns, UTC]").astype("int64") // 1_000_000
...
def check_filters(row: pd.Series, fvg_by_tf: dict, atr_lookup: pd.DataFrame,
    ...
    cur_close_ts = ts + TF_MS_1H          # cur bar close
    ...
    for tf, fvgs in fvg_by_tf.items():
        ...
        # FILTER 1 (IDM, canon 2026-07-14):
        # Любой wick-fill АКТИВНОЙ ЧАСТИ HTF FVG (первый/второй/N-й — все считаются).
        # Только полная mitigation (retire) исключает. Тронутая ранее часть — мёртвая, не IDM.
        # → Overlap проверяется против active_lo/hi (compute_active_bounds_at по истории 1h wicks).
        candidates = active_all[
            (active_all["direction"] == direction) &
            (active_all["zone_hi"] >= cur_l) &        # rough pre-filter по оригинал зоне
...
def main():
    ...
    print("[load] 1h bars for first-touch...")
    bars_1h = aggregate_1h_bars(CSV_1M)
    print(f"  {len(bars_1h):,} 1h bars")
    ...
    print(f"[load] ATR14 1h from CSV...")
    atr = load_atr_1h(CSV_1M)
    print(f"  {len(atr):,} 1h bars")

    print(f"[load] FVG intervals per TF + first-touch precompute...")
    fvg_by_tf = {}
    for tf in TFS:
        fdf = build_fvg_intervals(events, tf)
        fdf["first_wick_touch_ts"] = compute_first_wick_touch(fdf, bars_1h)
        fvg_by_tf[tf] = fdf
    ...
    for name, cohort in [("canonical", canonical), ("sweep-only", sweep_only)]:
        ...
        if len(cohort) == 0:
            ...
            continue
        _bts = bars_1h["ts_ms"].values.astype(np.int64)
        _bh = bars_1h["high"].values.astype(np.float64)
        _bl = bars_1h["low"].values.astype(np.float64)
