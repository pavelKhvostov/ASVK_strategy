"""Этап 3 — VC (Volume Confirmation) на 15m после 1h OB.

По методичке (стр 34-35):
  VC на X-1 (для 1h POI это 15m) = появление RB / FVG / SNR правильного направления
  после теста POI (X = 1h OB).

Здесь берём RB и FVG (SNR приблизим через ob_vc позже, если нужно).

Правила проверки:
  для LONG OB   : ищем bullish RB на 15m ИЛИ bullish FVG на 15m
                  born в окне [cur_close, cur_close + 48h]
                  zone пересекается с диапазоном 1h OB [cur.low, cur.high]
  для SHORT OB  : зеркально bearish (на 15m)
...
def main() -> None:
    ...
    rb_15m = build_element_born(events, "rb", "15m")
    fvg_15m = build_element_born(events, "fvg", "15m")
    snr_15m = build_element_born(events, "breaker_block", "15m")
    print(f"  RB 15m: {len(rb_15m):,}   FVG 15m: {len(fvg_15m):,}   SNR (breaker) 15m: {len(snr_15m):,}")
    print()
    ...
    for name, cohort in [("sweep-only", sweep_only), ("canonical", canonical)]:
        print(f"[check] {name}...")
        cohort["vc_rb"] = check_vc_batch(cohort, rb_15m, need_dir, window_ms)
        cohort["vc_fvg"] = check_vc_batch(cohort, fvg_15m, need_dir, window_ms)
        cohort["vc_snr"] = check_vc_batch(cohort, snr_15m, need_dir, window_ms)
    ...
    print()
    print("═" * 100)
    print(f"Этап 3 — VC на 15m после 1h OB в окне {SEARCH_WINDOW_H}h, zone overlaps [cur.low, cur.high]")
    print("═" * 100)
    ...
    print()
    row("VC RB 15m",                     sweep_only["vc_rb"],           canonical["vc_rb"])
    row("VC FVG 15m",                    sweep_only["vc_fvg"],          canonical["vc_fvg"])
    row("VC SNR (breaker_block) 15m",    sweep_only["vc_snr"],          canonical["vc_snr"])
    print()
