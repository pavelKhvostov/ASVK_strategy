"""OB canonical 1h — first-touch sweep semantics.
...
Sweep TFs: 1h, 2h, 4h, 6h, 12h. Проверяем first-touch по 1h bars.
"""
...
OUT_DIR = DATA_DIR / "liq_ob1h_vc"

TF_MS = {"1h": 3600*1000, "2h": 2*3600*1000, "4h": 4*3600*1000, "6h": 6*3600*1000, "12h": 12*3600*1000, "1d": 24*3600*1000, "2d": 2*24*3600*1000, "3d": 3*24*3600*1000, "1w": 7*24*3600*1000}
...
def build_config(symbol: str, end_date: str) -> dict:
    return {
        ...
        "sweep_tfs":          ["1h", "2h", "4h", "6h", "12h"],
        "sweep_semantics":   "first_touch_by_1h_bars",
...
def compute_first_breach_ts(
    ...
    """
    Для каждого фрактала возвращает ts_ms 1h-бара, который первым пробил его wick'ом.
...
def main() -> None:
    ...
    print(f"OB canonical 1h v1  symbol={symbol}  end={end_date}  cfg_hash={cfg_hash(CONFIG)}")
    ...
    for _, r in ob_born.iterrows():
        ...
        rows.append({
            ...
            "engulf":    engulf,
            "sweep_1h":  sweep_hits.get("1h", 0),
            "sweep_2h":  sweep_hits.get("2h", 0),
    ...
    print("═" * 60)
    print(f"OB canonical 1h — first-touch sweep ({symbol} {CONFIG['period_start']} → {CONFIG['period_end']})")
    print("═" * 60)
