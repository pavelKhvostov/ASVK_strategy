"""FVG_OB canonical 1h — IDM trigger (заменяет Liq_OB stage1+stage2).

Trigger (методичка AlexxxFlow, IDM = inducement):
  cur bar на 1h wick'ом взаимодействует с АКТИВНОЙ HTF FVG.
  → OB образован в момент wick-fill живой FVG (первый, второй, N-ный касание OK,
     пока FVG не полностью замитигирована).

Condition:
  Существует HTF FVG (4h/6h/12h/1d/2d/3d/1w) для которой:
...
OUT_DIR = DATA_DIR / "fvg_ob1h_vc"
CSV_DIR = DATA_DIR

HTF_FVG_TFS = ["4h", "6h", "12h", "1d", "2d", "3d", "1w"]
TF_MS_1H = 3600 * 1000
...
def aggregate_1h(csv_path: Path, period_end: str) -> pd.DataFrame:
    print(f"[agg 1h] {csv_path.name}...")
    t0 = time.time()
    ...
    print(f"[agg] 1h bars: {len(agg):,} ({time.time()-t0:.1f}s)")
    return agg


def load_atr_1h(bars_1h: pd.DataFrame) -> pd.DataFrame:
    df = bars_1h.copy()
...
def compute_first_wick_touch(fvgs: pd.DataFrame, bars_1h: pd.DataFrame) -> np.ndarray:
    """Для каждой FVG найти первый 1h бар, тронувший её range wick'ом.
...
def check_idm_prox(cur_open: int, cur_o: float, cur_h: float, cur_l: float, cur_c: float,
    ...
    """Проверить IDM trigger + PROX filter для одного OB.
    IDM (canon 2026-07-14): любой wick-fill АКТИВНОЙ ЧАСТИ HTF FVG (первый/второй/N — все).
    Только полная mitigation (retire) исключает. Overlap проверяется против active_lo/hi.
    PROX: свежая (не тронутая) FVG в направлении SL в 0.5×ATR."""
    cur_close_ts = cur_open + TF_MS_1H
    idm_hits_by_tf = {}
    prox_hits_by_tf = {}
    ...
    for tf, fvgs in fvg_by_tf.items():
        ...
        # IDM: overlap против active portion (не изначальная zone).
        candidates = active[
...
def main() -> None:
    ...
    print(f"FVG_OB canonical 1h  symbol={symbol}  end={end_date}  cfg_hash={cfg_hash(CONFIG)}")
    ...
    print(f"[filter] 1h OB born: {len(ob_born):,}")

    bars = aggregate_1h(CSV_1M, end_date)
    ...
    atr_lookup = load_atr_1h(bars)
    print(f"[atr] 1h ATR14: {len(atr_lookup):,}")
    ...
    print(f"[audit] scanning {len(ob_born):,} 1h OBs for IDM trigger ...")
    t0 = time.time()
    rows_idm_only = []
    rows_canonical = []
    n_no_bar = 0
    ...
    print("═" * 70)
    print(f"FVG_OB canonical 1h — IDM trigger ({symbol} {CONFIG['period_start']} → {CONFIG['period_end']})")
    print("═" * 70)
    print(f"total 1h OB born (with bars):       {n_all:>6,}")
