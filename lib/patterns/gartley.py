"""Gartley (BEARISH harmonic, SHORT) — X-A-B-C-D, D остаётся МЕЖДУ A и X
(ретрейс). Тонкая обёртка над harmonic_core.py, см. канон-верифицированный
G:\\Claude\\patterns\\gartley\\scan_gartley.py (254 уникальных паттерна,
BTCUSDT 1h 2020-01 → 2026-07, "полный эксперимент" 2026-07-25).

Коэффициенты (new-Table_0.1.jpg + веб-поиск tradingsim.com/tradingfinder.com/
trntrading.ai): AB/XA=0.55-0.68 (цель 0.618), D/XA=0.70-0.85 (цель 0.786).
...
PARAMS = HarmonicParams(
    ab_lo=0.500, ab_hi=0.720,
    bc_lo=0.382, bc_hi=0.920,
    cd_lo=1.130, cd_hi=1.800,
    d_xa_lo=0.750, d_xa_hi=0.850,
    d_beyond_x=False,
)


def compute_gartley(df_1h, symbol: str):
...
def main() -> None:
    ...
    hits, stats = compute_gartley(df_1h, args.symbol)
