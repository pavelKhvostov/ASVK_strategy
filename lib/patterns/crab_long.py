"""Crab BULLISH (LONG) — зеркало crab.py.
X=FL, A=FH, B=FL, C=FH, D=FL (PRZ = дно, вход LONG).
AB/XA=0.382-0.618, D/XA~1.618, D НИЖЕ X (самый глубокий экстеншн).
Коэффициенты идентичны SHORT Crab.
"""
...
def compute_crab_long(df_1h, symbol: str):
...
def main() -> None:
    ...
    hits, stats = compute_crab_long(df_1h, args.symbol)
