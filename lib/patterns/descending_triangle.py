"""Descending Triangle (BEARISH, SHORT) — Bulkowski Ch.65 "Triangles,
Descending". Тонкая обёртка над five_point_core.py, см. канон-верифицированный
G:\\Claude\\patterns\\descending_triangle\\scan_descending.py (12 уникальных
паттернов, BTCUSDT 1h 2020-01 → 2026-07).

Геометрия: опора (i1,i3,i5) плоская (|slope|<=0.05%/бар), сопротивление
(i2,i4) падает заметно (< -0.05%/бар — строго, чтобы не пересекаться с
Rectangle Top). TG (Table 65.10): height=H2-min(L1,L3,L5),
TG=sup_at(BR)-height (база — значение линии опоры в момент BR, не close[BR]).
...
def compute_descending_triangle(df_1h, symbol: str):
    return compute_five_point('descending_triangle', PATTERN_NAME, df_1h, symbol)
...
def main() -> None:
    ...
    hits, stats = compute_descending_triangle(df_1h, args.symbol)
