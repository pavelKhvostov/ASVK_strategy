"""Double Top (Adam&Adam variant, SHORT) — Bulkowski Ch.30-33 "Double Tops".
Геометрия портирована verbatim из канон-верифицированного
G:\\Claude\\patterns\\double_top\\scan_double_top.py (626 уникальных
паттернов, BTCUSDT 1h 2020-01 → 2026-07) на ASVK-portable контракт (df_1h
передаётся вызывающим кодом, FVG/MA — block5_common.apply_fvg_ma_filter).

Только Adam&Adam (оба пика острые/узкие) — фракталами Уильямса без анализа
формы пика остальные 3 варианта (Adam&Eve/Eve&Adam/Eve&Eve) неразличимы.

Геометрия (i1<i2<i3<i4, по cheat-sheet tradingaxe.com — НЕ 3 точки):
  i1,i3 = Williams N=2 FH (два пика, ~одна цена, допуск SAME_PRICE_TOL_PCT)
  i2,i4 = Williams N=2 FL (опора, ~один уровень, тот же допуск)
  Инвалидация — per-leg true extremum (H1/L2/H3/L4 каждый — настоящий
  экстремум СВОЕГО отрезка, не фрактал, мимо которого был больший экстремум)
  + между i1 и i4 потолок (max(H1,H3)) не пробит, между i2 и i4 опора не
  пробита раньше времени.
  BR = первое закрытие ниже линии опоры (i2->i4, экстраполирована) после i4.
  MAX_SPAN i1->i4 = 96ч, MAX_CONFIRM i4->BR = 24ч.

TG (Bulkowski Ch.30, Table 30.10, сверено по PDF: "subtract the lowest low
between the two highs from the highest peak... subtract the result from the
breakout price"):
  height = max(H1,H3) - min(L2,L4)
  TG = min(L2,L4) - height
    ...
    for i1 in i1_chunk:
        ...
        for i2 in i2_cands:
            ...
            for i3 in i3_cands:
                H3 = h[i3]
                ceiling = max(H1, H3)
                # leg i2->i3: true extremum (low не прокалывает опору, high не превышает H3)
                if h[i2:i3 + 1].max() > H3:
                    continue
                if l[i2:i3 + 1].min() < L2:
                    continue
                ...
                for i4 in i4_cands:
                    ...
                    # потолок не пробит (одна свеча может пробить если следующая вернулась)
                    invalid = False
                    for j in range(i1, i4 + 1):
                        if c[j] > ceiling * (1 + p.line_tol_pct / 100.0):
                            if j + 1 <= i4 and c[j + 1] <= ceiling * (1 + p.line_tol_pct / 100.0):
                                continue
                            invalid = True
                            break
                    if invalid:
                    ...
                    out.append({
                        ...
                        'H1': float(H1), 'L2': float(L2), 'H3': float(H3), 'L4': float(L4),
...
def detect_double_top_geometry(h: np.ndarray, l: np.ndarray, c: np.ndarray, ts: np.ndarray,
                                params: Optional[DoubleTopParams] = None) -> tuple[list[dict], dict]:
    if params is None:
...
def compute_double_top(df_1h: pd.DataFrame, symbol: str,
                        params: Optional[DoubleTopParams] = None) -> tuple[pd.DataFrame, dict]:
    if params is None:
        params = DoubleTopParams()
    ...
    geo, stats = detect_double_top_geometry(h, l, c, t_arr, params)
    ...
    final = apply_fvg_ma_filter(geo, t_arr, c, fvg_from_key='i4', i_br_key='i_breakout',
                                 h_arr=h, l_arr=l, n_bars=n_bars,
                                 hma_mhull=hma_mhull, hma_shull=hma_shull,
                                 wma50=wma50, stats=stats)
    ...
    for p in final:
        rows.append({
            ...
            "ts_breakout": p['ts_breakout'], "ts_i4": p['ts_i4'],
            "H1": p['H1'], "L2": p['L2'], "H3": p['H3'], "L4": p['L4'],
...
def print_stats(stats: dict) -> None:
    ...
    print(f"  passed_canon (full)   n={stats['passed_canon']}  ← DOUBLE_TOP", file=sys.stderr, flush=True)
...
def main() -> None:
    ...
    hits, stats = compute_double_top(df_1h, args.symbol)
    print_stats(stats)
