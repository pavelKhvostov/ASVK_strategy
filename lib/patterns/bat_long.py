"""Bat BULLISH (LONG) — зеркало bat.py.
X=FL, A=FH, B=FL, C=FH, D=FL (PRZ = дно, вход LONG).
AB/XA=0.382-0.50, D/XA~0.886, D выше X (ретрейс).
Коэффициенты идентичны SHORT Bat.
"""
...
def compute_bat_long(df_1h, symbol: str):
...
def main() -> None:
    ...
    hits, stats = compute_bat_long(df_1h, args.symbol)
