"""Falling Wedge 5-pt (LONG) — зеркало rising_wedge (Bulkowski Ch.36). Отличается от wedge.py (3H+2L).
...
Writes:
  G:\\ASVK\\data\\patterns\\falling_wedge_hits_{SYM}_{start}_{end}.parquet
"""
...
def compute_falling_wedge(df_1h, symbol):
...
def main() -> None:
    ...
    hits, stats = compute_falling_wedge(df_1h, args.symbol)
