"""Gartley BULLISH (LONG) — зеркало gartley.py.
X=FL, A=FH, B=FL, C=FH, D=FL (PRZ = дно, вход LONG).
AB/XA~0.618, D/XA~0.786, D выше X (ретрейс).
Коэффициенты идентичны SHORT Gartley.
"""
...
def compute_gartley_long(df_1h, symbol: str):
...
def main() -> None:
    ...
    hits, stats = compute_gartley_long(df_1h, args.symbol)
