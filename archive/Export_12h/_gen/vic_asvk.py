...
Верная копия ~/smc-warehouse/индикаторы/vic_asvk.py (WSL, read-only источник) —
скопирован 1-в-1 сюда только чтобы запустить с bundled ASVK python на свежих
данных G:\\ASVK\\data\\BTCUSDT_1m.csv (в WSL-копии график/ данные устарели, обрываются
19 июля). Оригинал в WSL не менялся и не трогался.
